...
public class AuthorizationMainListener implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(final ContextRefreshedEvent event) {
		Map<Integer, EOUserRole> userRoleMap = userRoleRepository.findAll().parallelStream()
				.collect(Collectors.toMap(EOUserRole::getPosition, Function.identity()));
		Map<String, EOUserAccount> userAccountMap = userAccountRepository.findAll().parallelStream()
				.collect(Collectors.toMap(EOUserAccount::getUsername, Function.identity()));
		if (upload) {
			...
			for (EOUserRole userRole : userRoleList) {
				...
				if (UserRole.ADMIN.getRoleType().equalsIgnoreCase(userRole.getRoleType())) {
					EOUserAccount eoUserAccount = userAccountMap.getOrDefault(eoUserRole.getRoleName(),
							new EOUserAccount());
					...
					eoUserAccount.setPassword((eoUserAccount.getPassword() == null ? eoUserRole.getRoleName()
							: eoUserAccount.getPassword()));
			...
			Map<String, EOMenuGroup> globalMenuGroupMap = globalMenuGroupRepository.findAll().stream()
					.collect(Collectors.toMap(EOMenuGroup::getIdenNo, Function.identity()));
			for (EOMenuGroup globalMenuGroup : globalMenuGroupList) {
				EOMenuGroup eoUserEndpoint = globalMenuGroupMap.getOrDefault(globalMenuGroup.getIdenNo(),
						globalMenuGroup);
			...
			Map<String, EOMenuItem> globalMenuItemMap = globalMenuItemRepository.findAll().stream()
					.collect(Collectors.toMap(EOMenuItem::getIdenNo, Function.identity()));
			for (EOMenuItem globalMenuItem : globalMenuItemList) {
				EOMenuItem eoGlobalMenuItem = globalMenuItemMap.getOrDefault(globalMenuItem.getIdenNo(),
						globalMenuItem);
			...
			Map<String, EORoleMenuGroup> roleMenuGroupMap = roleMenuGroupRepository.findAll().parallelStream()
					.collect(Collectors.toMap((userRoleMenuGroup) -> getRoleMenuGroupKey(userRoleMenuGroup),
							Function.identity()));
			...
			Map<String, EORoleMenuItem> deleteRoleMenuItemMap = roleMenuItemRepository.findAll().parallelStream()
					.collect(Collectors.toMap((userRoleMenuItem) -> getRoleMenuItemKey(userRoleMenuItem),
							Function.identity()));
			Map<String, EORoleMenuItem> checkRoleMenuItemMap = roleMenuItemRepository.findAll().parallelStream()
					.collect(Collectors.toMap((userRoleMenuItem) -> getRoleMenuItemKey(userRoleMenuItem),
							Function.identity()));
			...
			Map<String, EOHeaderItem> headerItemMap = headerItemRepository.findAll().parallelStream()
					.collect(Collectors.toMap((headerItem) -> headerItem.getIdenNo(), Function.identity()));
			...
			Map<String, EORoleHeaderItem> roleHeaderItemMap = roleHeaderItemRepository.findAll().parallelStream()
					.collect(Collectors.toMap((userRoleHeaderItem) -> getRoleHeaderItemKey(userRoleHeaderItem),
							Function.identity()));
			...
			for (EORoleHeaderItem roleHeaderItem : userRoleHeaderItemList) {
				try {
					...
					EORoleHeaderItem eoRoleHeaderItem = roleHeaderItemMap.getOrDefault(roleHeaderItemKey,
							roleHeaderItem);
